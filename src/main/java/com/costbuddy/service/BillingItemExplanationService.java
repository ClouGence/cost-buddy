...
public class BillingItemExplanationService {
    ...
    public BillingItemExplanationDO get(Long id) {
    ...
    private String buildPromptContext(BillingAuditItemDO item, List<BillingAuditItemResourceResponse> resources) {
        ...
        builder.append("阿里云文档检索关键词: ")
            .append(value(item.getProductName())).append(' ')
            .append(value(item.getBillingItem())).append(' ')
            .append(value(item.getBillingItemCode())).append('\n');
        ...
        resources.stream().limit(12).forEach(resource -> builder
            .append("- resourceId=").append(value(resource.getInstanceId()))
            .append(", name=").append(value(resource.getInstanceName()))
            .append(", region=").append(value(resource.getRegion()))
            .append(", resourceGroup=").append(value(resource.getResourceGroup()))
            .append(", usage=").append(resource.getUsageAmount()).append(' ').append(value(resource.getUsageUnit()))
            .append(", amount=").append(resource.getPretaxAmount()).append(' ').append(value(resource.getCurrency()))
            .append(", billingType=").append(value(resource.getBillingType()))
            .append('\n'));
    ...
    private String aliyunHelpSearchUrl(BillingAuditItemDO item) {
        String keyword = String.join(" ",
            ...
            value(item.getBillingItem()),
            value(item.getBillingItemCode())
        ).replace("-", "").trim();
        if (keyword.isBlank()) {
            keyword = "阿里云 计费项";
        }
